        ...
        stage('Zipping Files') {
            steps {
                script {
                    ...
                    bat """
                        powershell.exe -Command "if (Test-Path '${path_zipfile}') { Remove-Item '${path_zipfile}' }"
                        powershell.exe -Command "Compress-Archive -Path @(Get-ChildItem '${includedfile}' | Where-Object {${tenantstobeexcluded.collect { \"'\$_'\" }.join(', ') } -notcontains \$_.Name }) -DestinationPath '${path_zipfile}'"
                    """
                    /*bat """
